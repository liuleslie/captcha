...
// Deduplication: track last save time+hostname per tab to prevent double-saves
// (e.g. interval timer and captcha-disappeared firing within seconds of each other).
let lastSaveByTab = {}; // tabId → { at: timestamp, hostname: string }
...
browser.tabs.onRemoved.addListener(async (tabId) => {
  ...
  if (hasImages || hasCursors) {
    flushSessionForTab(tabId, "tab-close");
  }
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "get-popup-data") {
    ...
    getPopupData(queryTabId)
      ...
      .catch(() => sendResponse({ recentSession: null, totalSessions: 0 }));
    return true;
  }
...
async function saveSession(tabId, sessionData) {
  ...
  // Deduplicate: skip if we saved for the same hostname on this tab within 15 seconds.
  // Prevents double-saves when the interval timer and captcha-disappeared trigger overlap.
  const hostname = sessionData.hostname || "";
  const prev = lastSaveByTab[tabId];
  if (prev && prev.hostname === hostname && (Date.now() - prev.at) < 15000) {
  ...
  lastSaveByTab[tabId] = { at: Date.now(), hostname };
...
async function exportZip(sessionIds) {
  ...
  for (const sessionId of sessionIds) {
    ...
    // session.json (without binary data)
    const sessionJson = JSON.stringify({ ...session }, null, 2);
    folder.file("session.json", sessionJson);
