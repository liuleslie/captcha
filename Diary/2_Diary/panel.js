...
async function init() {
  ...
  if (data.totalSessions === 0) {
    countEl.textContent = "No recordings yet";
  } else {
    countEl.textContent = `${data.totalSessions} recording${data.totalSessions !== 1 ? "s" : ""} in archive`;
  }
  ...
  if (!data.recentSession) {
    ...
    if (data.hasActiveImages) {
      const saveBtn = document.getElementById("save-now-btn");
      saveBtn.classList.remove("hidden");
      saveBtn.addEventListener("click", async () => {
        saveBtn.disabled = true;
        saveBtn.textContent = "Saving…";
        await browser.runtime.sendMessage({ action: "manual-flush", tabId: tab.id });
        window.close();
      });
    }
    return;
